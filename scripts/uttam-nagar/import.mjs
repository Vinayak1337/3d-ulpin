...
const source=resolve(process.argv[2]||resolve(root,'fixtures/uttam-nagar'));
const stage=process.argv[3]||'all';
assert(['all','context','details','residents'].includes(stage));
const runtime=resolve(root,'.runtime/uttam-nagar');
...
async function layer(key,file,kind,synthetic,areaId,extra={}){
 ...
 if(previous)pkg=await api(`/import-packages/${previous.id}`);
  ...
  form.set('namespace',`uttam-nagar-20260917:${key}`);
  form.set('name',synthetic?'Uttam Nagar | SYNTHETIC rooms, residents and conflicts':'Uttam Nagar | Google footprints + OSM roads');
  form.set('worldStatus',synthetic?'synthetic':'observed');
  const mapped={idField:'source_id',nameField:'name',kind,geometryRole:kind==='building'?'observed_roof_projection':'unknown',identifierFields:kind==='building'?['full_plus_code','confidence','google_area_m2','google_tile','google_csv_row','source_notice','reference_source_id']:['osm_way_id','highway','width','lanes','source_notice','assumed_width_m','expected_target_overlap_m2'],...extra};
...
async function residents(){
 for(const [buildingId,entry] of Object.entries(state.buildings)){
  ...
  for(const occupant of entry.occupants){
   ...
   const body={...bodyOnly(record),use:occupant.common?'common':'apartment',rights:[{party:occupant.party,type:'shared_use',evidence:{sourceId,locator:`Fictional occupant schedule: ${occupant.alias}. Demonstration shared use only; no real resident or ownership.`}}]};
