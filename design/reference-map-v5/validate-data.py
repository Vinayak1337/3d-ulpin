...
def main():
 ...
 # Compute all contacts/conflicts, then require exact declared evidence instead of rejecting intended cases.
 expected={};contacts={};tol=1e-7
 for i,a in enumerate(buildings):
  for b in buildings[i+1:]:
   aid,bid=a['id'],b['id'];hit=polys[aid].intersection(polys[bid]);length=polys[aid].boundary.intersection(polys[bid].boundary).length
   if hit.area>tol:expected[('BUILDING_OVERLAP',aid,bid)]=hit;contacts[(aid,bid)]=('positive_area_overlap',hit.area,length)
   elif length>tol:contacts[(aid,bid)]=('shared_boundary',0,length)
 roads=[o for o in d['objects'] if o['type']=='road']
 for o in buildings:
  bid=o['id'];pid=o['attributes']['parcel_id'];diff=polys[bid].difference(polys[pid])
  if diff.area>tol:expected[('OUTSIDE_PARCEL',bid,pid)]=diff
  for road in roads:
   hit=polys[bid].intersection(polys[road['id']])
   if hit.area>tol:expected[('ROAD_OVERLAP',bid,road['id'])]=hit
 declared={}
 for issue in d['issues']:
  if issue['code'] not in ['BUILDING_OVERLAP','OUTSIDE_PARCEL','ROAD_OVERLAP']:continue
  key=(issue['code'],*issue['objectIds']);check(key not in declared,'Duplicate spatial issue');check(key in expected,'Declared conflict does not exist');ev=issue['evidence'];actual=expected[key];declared[key]=ev['areaM2']
  check(abs(actual.area-ev['areaM2'])<tol,'Conflict area mismatch');check(shape(ev['intersectionGeometry']).symmetric_difference(actual).area<tol,'Conflict region mismatch')
  check(ev['frameId']==d['frames'][0]['id'] and ev['verticalDatum']==d['frames'][0]['verticalDatum'],'Conflict frame mismatch');check(ev['legalFinding'] is False,'Spatial check implies legal determination')
  check(ev['computedFromGeometryIds']==[geoms[oid]['id'] for oid in issue['objectIds']],'Conflict input provenance mismatch')
  check(all(ref['geometryId']==geoms[ref['objectId']]['id'] and ref['version']==geoms[ref['objectId']]['version'] for ref in ev['geometryRefs']),'Conflict geometry version mismatch')
  if issue['code']=='BUILDING_OVERLAP':
   ga,gb=[geoms[oid] for oid in issue['objectIds']];overlap=max(0,min(ga['baseElevationM']+ga['heightM'],gb['baseElevationM']+gb['heightM'])-max(ga['baseElevationM'],gb['baseElevationM']))
   check(overlap>0 and abs(ev['overlapHeightM']-overlap)<tol and abs(ev['volumeM3']-actual.area*overlap)<tol,'Building overlap volume mismatch')
  else:check(ev['volumeM3'] is None and ev['verticalRelation']=='surface','Planar parcel/road conflict cannot assert volume')
 check(set(declared)==set(expected),'Undeclared computed conflict')
 sc=d['spatialChecks'];recorded={(p['fromId'],p['toId']):p for p in sc['buildingPairs']};check(set(recorded)==set(contacts),'Spatial pair inventory drift')
 for key,(classification,area,length) in contacts.items():
  item=recorded[key];check(item['classification']==classification and abs(item['areaM2']-area)<tol and abs(item['sharedBoundaryLengthM']-length)<tol,'Shared boundary/positive overlap classification drift')
 check(any(v[0]=='shared_boundary' for v in contacts.values()),'Fixture must include valid shared walls')
 check(any(k[0]=='BUILDING_OVERLAP' for k in expected),'Fixture must include a positive building overlap')
 check(any(k[0]=='ROAD_OVERLAP' for k in expected),'Fixture must include a positive road overlap')
 check(any(k[0]=='OUTSIDE_PARCEL' for k in expected),'Fixture must include parcel encroachment')
 for collection,code,bkey,okey in [('parcelEncroachments','OUTSIDE_PARCEL','buildingId','parcelId'),('roadOverlaps','ROAD_OVERLAP','buildingId','roadId')]:
  recorded={ (code,i[bkey],i[okey]):i for i in sc[collection] }
  check(set(recorded)=={k for k in expected if k[0]==code},'Spatial check inventory mismatch')
  for key,item in recorded.items():check(abs(item['areaM2']-expected[key].area)<tol and shape(item['geometry']).symmetric_difference(expected[key]).area<tol,'Spatial check geometry mismatch')
 union_members=[]
 for block in sc['blocks']:
  union_members.extend(block['objectIds']);actual=unary_union([polys[oid] for oid in block['objectIds']]);check(shape(block['unionGeometry']).symmetric_difference(actual).area<tol and abs(block['areaM2']-actual.area)<tol,'Union block geometry mismatch')
 check(sorted(union_members)==sorted(o['id'] for o in buildings),'Union grouping loses/duplicates building identity')
 check(json.loads((DATA/'spatial-checks.json').read_text())==sc and json.loads((PKG/'spatial-checks.json').read_text())==sc,'Stored spatial oracle differs')
 for point in d['sceneDecoration']['trees']:check(not any(poly.contains(Point(point[:2])) for oid,poly in polys.items() if idx['objects'][oid]['type']=='building'),'Tree trunk inside a building')
 ...
 check(len([i for i in d['issues'] if i['code']=='MISSING_BUILDING_HEIGHT'])==1 and next(i for i in d['issues'] if i['code']=='MISSING_BUILDING_HEIGHT')['objectIds']==['B12'],'Pending height issue drift')
 for s in d['sources']:
 ...
 for p in sorted(PKG.glob('*.geojson')):
  ...
  for f in source['features']:
   ...
   reconstructed={'type':f['geometry']['type'],'coordinates':coords};expected_geometry={'type':geoms[oid]['type'],'coordinates':geoms[oid]['coordinates']};error=shape(reconstructed).hausdorff_distance(shape(expected_geometry));max_error=max(error,max_error);check(error<=1e-6,'Geometry round-trip error')
