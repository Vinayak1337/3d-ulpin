...
export async function POST(request:Request){try{localRequest(request);const name=decodeURIComponent(request.headers.get('x-file-name')??'');const bytes=await readPackageBody(request);const saved=await saveSpatialDataset(name,bytes);await ensureDatasetIdentifiers(saved.id);return Response.json(saved,{status:201,headers:{'Cache-Control':'no-store'}});}catch(e){return datasetError(e);}}
