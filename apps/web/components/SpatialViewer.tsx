...
import styles from "./SpatialViewer.module.css";
...
export default function SpatialViewer({ model, selectedId, onSelect, floor, isolate, explode, finding }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      (Cesium.buildModuleUrl as typeof Cesium.buildModuleUrl & { setBaseUrl: (url: string) => void }).setBaseUrl("/cesium/");
      instance = new Cesium.Viewer(container.current, {
        ...
        shadows: true, contextOptions: { webgl: { alpha: true, antialias: true } },
      });
      instance.scene.backgroundColor = Cesium.Color.fromCssColorString("#f2f0eb");
      instance.scene.fog.enabled = false;
      instance.scene.shadowMap.softShadows = true;
      instance.scene.shadowMap.size = 1024;
      instance.scene.shadowMap.maximumDistance = 90;
      instance.scene.shadowMap.darkness = 0.4;
      instance.scene.shadowMap.normalOffset = true;
      // These bias fields belong to the pinned Cesium engine. Millimetre-scale
      // architectural details need less normal displacement than globe meshes.
      const shadowBias = (instance.scene.shadowMap as Cesium.ShadowMap & { _primitiveBias: { depthBias: number; normalOffsetScale: number } })._primitiveBias;
      shadowBias.depthBias = 0.00012;
      shadowBias.normalOffsetScale = 0.06;
      ...
      instance.scene.renderError.addEventListener((_scene, cause: Error) => {
        setError(cause.message || "The 3D scene could not be rendered.");
      });
      instance.cesiumWidget.creditContainer.setAttribute("aria-label", "Cesium attribution");
