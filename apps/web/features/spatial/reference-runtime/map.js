...
export function mountMap(container, data, { onSelect, onRegister, onReview, onWorkspace, onFloorSelect, detail } = {}) {
  ...
  </aside><section class="bm-stage" aria-label="Interactive synthetic neighborhood map">
  ...
  function renderList(query='') {
    ...
    $('.bm-property-list').innerHTML=matches.length?matches.map(hit=>{const o=objectById.get(hit.buildingId),record=identityFor(hit.objectId),parcelIds=identityFor(o)?.twoDIds.map(a=>a.value)||[];return `<button class="bm-property-row" data-select="${esc(hit.buildingId)}" ${hit.floorId?`data-match-floor="${esc(hit.floorId)}"`:''} aria-pressed="${hit.buildingId===state.selectedId&&(!hit.floorId||state.floor===hit.floorId)}">${icon(hit.floorId?'floor':'building')}<span><strong>${esc(record?.primary||o.id)}${conflictsFor(o.id).length?'<i class="bm-conflict-dot" title="Computed spatial conflict"></i>':''}</strong><small>${esc(hit.label)}${hit.floorId?` · ${esc(o.label)}`:''}</small>${!hit.floorId&&parcelIds.length?`<small class="bm-list-parcel">2D · ${esc(parcelIds.join(', '))}</small>`:''}</span>${hit.buildingId===state.selectedId?icon('check'):icon('chevron')}</button>`;}).join(''):'<p class="bm-empty">No match. Search a 2D ULPIN, building or floor 3D ULPIN, name or address.</p>';
  }
  ...
  function renderInspector() {
    ...
    const identity=identityFor(o),parcelLabels=identity?.twoDIds.map(a=>a.value)||[];
    ...
    const occupantContent=`<section class="bm-occupant-section"><h3>${floor?'Floor residents':'Residents'} <span>(${occupants.length} supplied records)</span></h3>${occupants.length?`<p class="bm-detail-note">${occupants.every(r=>r.classification==='synthetic')?'Fictional demonstration people. No ownership claim.':'Supplied occupancy records; not an ownership determination.'}</p>${occupants.map(r=>`<article class="bm-occupant-row"><div><strong>${esc(r.name)}</strong><small>${esc(r.role)} · ${esc(r.classification)}</small></div><span>${esc(primaryId(r.unitId||r.floorId||r.objectId))}</span></article>`).join('')}`:'<p class="bm-detail-note">No resident records supplied.</p>'}</section>`;
    const address=o.attributes?.address || o.address || 'Synthetic property record';
    ...
    let content='';
    if(state.inspectorTab==='overview')content=`${summary}${!Number.isFinite(geo?.baseElevationM)?'<p class="bm-uncertainty">Base elevation not supplied · shown at display ground. Vertical placement is illustrative.</p>':''}${findings}<details class="bm-detail"><summary>Record details</summary><dl class="bm-facts"><dt>3D ULPIN</dt><dd>${esc(identity.primary)}<small class="bm-id-qualification">Internal${identity.synthetic?' · fictional':''}</small></dd><dt>Parcel 2D ULPIN</dt><dd>${parcelLabels.length?esc(parcelLabels.join(', ')):'Not supplied'}</dd><dt>Level records</dt><dd>${floors.length?`${floors.length} supplied`:'Not supplied'}</dd><dt>Sources</dt><dd>${sources.length} linked files</dd><dt>Geometry revision</dt><dd>${esc(geo?.version??'Unavailable')}</dd><dt>Vertical reference</dt><dd>${esc(geo?.verticalDatum||'Unknown')}</dd></dl></details><button class="bm-inline-link" data-inspector-tab="floors">Explore floors & section ${icon('arrow')}</button><details class="bm-detail bm-resident-details"><summary>Resident records (${occupants.length}) ${icon('chevron')}</summary>${occupantContent}</details>`;
    if(state.inspectorTab==='floors')content=`<section class="bm-model-controls"><div class="bm-section-heading"><h3>Explore the building</h3><button class="bm-text-button" data-action="focus" aria-label="Focus selected building">${icon('target')}Focus</button></div><label class="bm-field-label" for="bm-floor">Floor</label><select id="bm-floor" class="bm-select" ${!floors.length?'disabled':''}><option value="all">${floors.length?`All ${floors.length} levels`:'No floor geometry supplied'}</option>${floors.map(f=>`<option value="${esc(f.id)}" ${state.floor===f.id?'selected':''}>${esc(f.label)} · ${esc(primaryId(f))}</option>`).join('')}</select><div class="bm-floor-options" ${state.floor==='all'?'hidden':''}><div class="bm-segment"><button data-floor-mode="below" ${!floorPlaced?'disabled':''} aria-pressed="${state.floorMode==='below'}">Hide above</button><button data-floor-mode="isolate" ${!floorPlaced?'disabled':''} aria-pressed="${state.floorMode==='isolate'}">Isolate floor</button></div><p>Floor 3D ULPIN: ${floor?esc(primaryId(floor)):''}</p>${floor&&!floorPlaced?'<p class="bm-uncertainty">Schedule record only. No floor geometry was supplied; the whole building remains context.</p>':''}${spaces.length?`<label class="bm-field-label" for="bm-space">Space on this floor</label><select id="bm-space" class="bm-select"><option value="none">Choose a space (${spaces.length})</option>${spaces.map(space=>`<option value="${esc(space.id)}" ${state.space===space.id?'selected':''}>${esc(space.label)}</option>`).join('')}</select>`:''}</div><label class="bm-switch-row"><span>${icon('floor')}Separate floors</span><input type="checkbox" class="bm-switch" data-control="explode" ${state.explode?'checked':''} ${placedFloors.length<2?'disabled':''}/></label><label class="bm-switch-row"><span>${icon('layers')}Section cut</span><input type="checkbox" class="bm-switch" data-control="section" ${state.section?'checked':''} ${!hasVerticalReference?'disabled':''}/></label>${hasHeight&&!hasVerticalReference?'<p class="bm-uncertainty">Base elevation not supplied. Benchmark section elevations are unavailable.</p>':''}<div class="bm-section-slider" ${!state.section?'hidden':''}><label for="bm-cut">Cut elevation <output>${state.sectionHeight.toFixed(1)} m</output></label><input id="bm-cut" type="range" min="${minCut}" max="${Math.max(maxCut,minCut+.1)}" step="0.1" value="${state.sectionHeight}"/><p>Elevation in the named local benchmark. Applies to the scene.</p></div>${!hasHeight?'<p class="bm-uncertainty">Height is not supplied. Only the known footprint is shown.</p>':''}</section><div class="bm-record-section">${floors.map(f=>`<button class="bm-level-row" data-floor-select="${esc(f.id)}"><span>${esc(f.label)}<small>${esc(primaryId(f))}</small></span><strong>${Number.isFinite(geometryFor(f)?.baseElevationM)?`${geometryFor(f).baseElevationM.toFixed(1)} m`:'Unplaced'}</strong></button>`).join('')}</div>${occupantContent}`;
    ...
    inspector.innerHTML=`<div class="bm-inspector-title"><strong>Building</strong><div><button class="bm-icon-button" data-action="previous-property" aria-label="Previous building">‹</button><button class="bm-icon-button" data-action="next-property" aria-label="Next building">›</button><button class="bm-icon-button" data-action="close-inspector" aria-label="Close inspector">${icon('close')}</button></div></div><div class="bm-inspector-heading"><div class="bm-scene-thumbnail"><img alt="Current interactive block view"/>${conflicts.length?'<span class="bm-conflict-badge">! Conflict detected</span>':'<span class="bm-draft-badge">Draft geometry</span>'}</div><div class="bm-id-kind">3D ULPIN · internal${identity.synthetic?' · fictional':''}</div><h2 title="${esc(identity.primary)}">${esc(identity.primary)}</h2><p class="bm-building-name">${esc(o.label)}</p><p>${esc(address)}</p>${parcelLabels.length?`<p class="bm-heading-parcel">2D · ${esc(parcelLabels.join(', '))}</p>`:''}<div class="bm-property-chips"><span>${icon('building')}${esc(o.attributes?.use||'Building')}</span><span>${icon('floor')}${floors.length?`${floors.length} levels`:'Levels unknown'}</span><span class="bm-source-status">Synthetic</span></div></div><div class="bm-inspector-tabs" role="tablist" aria-label="Building details">${[['overview','Overview'],['floors','Floors'],['parcels','Parcels'],['sources','Sources'],['history','History']].map(([key,label])=>`<button data-inspector-tab="${key}" role="tab" aria-selected="${state.inspectorTab===key}">${label}</button>`).join('')}</div><div class="bm-inspector-scroll">${content}</div><div class="bm-inspector-footer"><div class="bm-footer-links">${onRegister?`<button class="bm-button" data-action="register">${icon('source')}Open register</button>`:''}${onWorkspace?`<button class="bm-button" data-action="workspace">${icon('arrow')}Workspace</button>`:''}</div>${onReview?`<button class="bm-primary" data-action="review">${icon('search')}Review findings</button>`:''}</div>`;
  ...
  function setRailOpen(open){
    ...
    if(state.railOpen&&narrowLayout())setInspectorOpen(false);
  }
  ...
  listen(container,'keydown',event=>{
    ...
    if(event.key==='Escape'){setRailOpen(false);closePopovers();state.floor='all';state.space='none';state.explode=false;state.section=false;renderInspector();updateScene();canvasHost.focus();live('Floor tools reset.');}
  ...
  $('.bm-world-labels').innerHTML=mapLabels.map(o=>`<span class="bm-world-label ${o.object.type==='open_area'?'bm-world-public':o.object.type==='building'?'bm-world-building':o.object.type==='utility'?'bm-world-utility':''}">${esc(o.object.type==='building'?primaryId(o.object):o.object.label)}</span>`).join('');
  ...
  function restoreState(saved) {
    ...
    for(const key of ['explode','section','underground','floorPlates'])if(typeof saved[key]==='boolean')state[key]=saved[key];
    ...
    state.inspectorTab=['overview','floors','parcels','sources','history'].includes(saved.inspectorTab)?saved.inspectorTab:'overview';state.labels=saved.labels!==false;container.querySelectorAll('[data-action="labels"]').forEach(button=>button.setAttribute('aria-pressed',String(state.labels)));setRailTab(saved.railTab||'layers');
