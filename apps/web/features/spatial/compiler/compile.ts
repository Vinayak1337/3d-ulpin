...
import { enuToEcef, geometryBounds, metricTopologyIssue, stableEncode, validateSpatialSnapshot, type SpatialSnapshot } from "@ulpin/contracts";
...
export function compileSpatialSnapshot(snapshot: SpatialSnapshot, baseUrl: string): CompiledPublication {
    ...
    for (const input of inputs) {
        ...
        if (!["Polygon", "MultiPolygon", "Point"].includes(r.geometry.type))
    ...
    const supportedKinds = new Set(["building", "building_part", "parcel", "road", "public_land", "terrain", "vegetation"]);
