...
export function encodeGlb(groups: Map<number, MeshData>, features: RenderFeature[]): Buffer {
    ...
    for (const [material, mesh] of [...groups].sort((a, b) => a[0] - b[0])) {
        ...
        primitives.push({ attributes: { POSITION: position, NORMAL: normal, _FEATURE_ID_0: feature }, indices, material, mode: 4, extensions: { EXT_mesh_features: { featureIds: [{ featureCount: features.length, attribute: 0, propertyTable: 0, label: "entity" }] } } });
    }
    ...
    const doc = { asset: { version: "2.0", generator: "3d-ulpin shared scene compiler" }, scene: 0, scenes: [{ nodes: [0] }], nodes: [{ mesh: 0 }], meshes: [{ primitives }], buffers: [{ byteLength: offset }], bufferViews, accessors,
        materials: materials.map(m => ({ name: m.name, pbrMetallicRoughness: { baseColorFactor: m.color, roughnessFactor: m.roughness, metallicFactor: m.metallic }, doubleSided: false })),
