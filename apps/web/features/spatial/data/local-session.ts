...
export function localOrbitState(input:unknown,frameId:string,snapshotDigest:string):LocalOrbitState|null{
  ...
  return {kind:'engineering-orbit',frameId,snapshotDigest,view:{selectedId:typeof v.selectedId==='string'?v.selectedId:null,activeConflictId:typeof v.activeConflictId==='string'?v.activeConflictId:null,mode:v.mode==='2d'?'2d':'3d',floor:text('floor','all'),space:text('space','none'),floorMode:text('floorMode','below'),floorPlates:v.floorPlates===true,inspectorOpen:v.inspectorOpen!==false,labels:v.labels!==false,railTab:v.railTab==='properties'?'properties':v.railTab==='findings'?'findings':'layers',inspectorTab,explode:v.explode===true,section:v.section===true,sectionHeight:typeof v.sectionHeight==='number'&&Number.isFinite(v.sectionHeight)?v.sectionHeight:100,underground:v.underground===true,layers,camera:{position:[...camera.position],target:[...camera.target],perspectiveOffset:[...camera.perspectiveOffset],zoom:camera.zoom,orthoHalfHeight:camera.orthoHalfHeight}}};
}
