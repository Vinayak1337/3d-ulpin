...
export default function FindingsTray({ block }: { block: BlockController }) {
  ...
  return (
    <section className="v2-findings-tray" aria-label="Block findings">
      <header>
        <div>
          <Icon name="warning" />
          <h2>Findings</h2>
      ...
      {check?.stale && (
        <p className="v2-stale-check" role="status">
      ...
      <div className="v2-findings-list">
        {filtered.map((f) => (
          ...
          >
            <span
              className={`v2-finding-icon ${f.category === "geometric" ? "v2-finding-icon--danger" : ""}`}
            >
              <Icon name={f.category === "geometric" ? "warning" : "info"} />
            </span>
            <span className="v2-finding-copy">
