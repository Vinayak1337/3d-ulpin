...
export function SearchDialog({
  ...
  const recent = useV2Store((state) => state.recentProperties),
    selectedArea = useV2Store((state) => state.selectedAreaId);
    ...
    <Dialog open={open} onClose={onClose} title="Find a property">
      <div className="v2-search-input">
        ...
        <input
          ...
          placeholder="Property ID, source ID or building ID"
          value={query}
      ...
      {!editing && !settled ? (
        <div className="v2-search-results">
          <p className="v2-eyebrow">Recently opened</p>
          {recent.map((property) => (
            <button
