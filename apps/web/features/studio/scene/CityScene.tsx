...
import Batch from './InstanceBatch';
import BuildingEnvelope,{ArchitectureParts} from './BuildingEnvelope';
import {rectangleDifference} from '../data/display-geometry';
import { useGroundMaps, EnvironmentLighting, Grounding } from './Appearance';
...
import { district, computeFindings, utilityClearance } from '../data/district';
...
function Architecture({selected,hidden,underground,onSelect,onHover}:{selected:string|undefined;hidden:string|undefined;underground:boolean;onSelect:(id:string)=>void;onHover:(id:string|null)=>void}){
  ...
  const selectedBuilding=district.buildings.find(b=>b.id===selected);
  const issue=selectedBuilding&&computeFindings(selectedBuilding).some(f=>f.severity==='critical');
  return <ArchitectureParts parts={Object.fromEntries(parts.map(p=>[p.name,p.items]))} hidden={hidden} underground={underground} onPick={onSelect} onHover={onHover} highlight={selected&&!hidden?{id:selected,tint:issue?'#e67468':'#77a78c',strength:issue?.5:.16}:undefined}/>;
...
function Surface({layers,underground,onSelect}:{layers:Layers;underground:boolean;onSelect:(id:string)=>void}) {
  ...
  return <group>
    ...
    {district.parks.map(p=><group key={p.id} position={[p.x,0,p.z]}>
      <mesh receiveShadow position={[0,.3,0]} rotation={[-Math.PI/2,0,0]} raycast={noRay}><planeGeometry args={[p.width,p.depth]}/><meshStandardMaterial map={maps.grass} normalMap={maps.grassNormal} normalScale={[.24,.24]} color="#b8d797"/></mesh>
...
function RoadText({text,x,z,vertical=false,size=4.4,color='#f0f1e8'}:{text:string;x:number;z:number;vertical?:boolean;size?:number;color?:string}) {
...
function Labels(){const group=useRef<THREE.Group>(null!);useFrame(({camera})=>{group.current.visible=camera.zoom>1.4;});return <group ref={group}>{district.roads.flatMap(r=>district.blocks.filter(b=>r.axis==='x'?Math.abs(b.z-r.z)<40:Math.abs(b.x-r.x)<40).map(b=>r.axis==='x'?b.x:b.z).filter((v,i,a)=>a.indexOf(v)===i).map(p=><RoadText key={`${r.id}-${p}`} text={r.name} x={r.axis==='x'?p:r.x} z={r.axis==='z'?p:r.z} vertical={r.axis==='z'}/>))}{district.parks.map(p=><Html key={p.id} position={[p.x,2,p.z]} center wrapperClass="map-label-wrapper" zIndexRange={[12,0]}><div className="studio-park-label">{p.name}<small>Synthetic public land</small></div></Html>)}</group>;}

function Utilities({layers,underground,selected}:{layers:Layers;underground:boolean;selected:Building|null}) {
  const nearest=selected?district.utilities.filter(u=>u.kind==='Water').map(u=>({u,gap:utilityClearance(selected,u)})).sort((a,b)=>a.gap-b.gap)[0]:null;
  ...
  })}{selected&&nearest&&nearest.gap<8&&layers.water&&layers.labels&&!underground&&<Html position={[selected.x+selected.width/2+7,1,nearest.u.points[0][2]===nearest.u.points[1][2]?nearest.u.points[0][2]:selected.z]} center wrapperClass="map-label-wrapper" zIndexRange={[14,0]}><div className="studio-network-label">Water pipeline<small>Centre depth {nearest.u.depth} m</small></div></Html>}</group>;
}
...
function CutawayBuilding({b,exploded,floor,onFloor,onUnit}:{b:Building;exploded:boolean;floor:number|null;onFloor:(n:number|null)=>void;onUnit?:(id:string)=>void}) {
  ...
  return <group position={[b.x,.4,b.z]}><group position={[0,-.4,0]}><BuildingEnvelope building={local} cutaway spacing={exploded?5.9:b.floorHeight} maxFloor={floor!==null&&!exploded?floor:undefined}/></group>{Array.from({length:b.floors},(_,f)=>{
...
function Selection({b,exploded,floor,onFloor,layers,onUnit}:{b:Building;exploded:boolean;floor:number|null;onFloor:(n:number|null)=>void;layers:Layers;onUnit?:(id:string)=>void}) {
  ...
  const p=b.parcel;
  const outside=rectangleDifference(b,p);
  return <group>
    ...
    {(exploded||floor!==null)?<CutawayBuilding b={b} exploded={exploded} floor={floor} onFloor={onFloor} onUnit={onUnit}/>:<mesh position={[b.x,b.height/2+.42,b.z]} raycast={noRay}>
      <boxGeometry args={[b.width+.14,b.height+.1,b.depth+.14]}/><meshStandardMaterial color={color} transparent opacity={bad?.12:.06} depthWrite={false}/><Edges color={color} lineWidth={2}/>
    </mesh>}
    {layers.findings&&<>
      {outside.map((patch,index)=><group key={`parcel-patch-${index}`}><mesh position={[patch.x,.53,patch.z]} rotation={[-Math.PI/2,0,0]} raycast={noRay}><planeGeometry args={[patch.width,patch.depth]}/><meshBasicMaterial color="#f1a34c" transparent opacity={.85} side={THREE.DoubleSide}/></mesh><mesh position={[patch.x,b.height+.71,patch.z]} rotation={[-Math.PI/2,0,0]} raycast={noRay}><planeGeometry args={[patch.width,patch.depth]}/><meshBasicMaterial color="#e84836" transparent opacity={.62} side={THREE.DoubleSide}/></mesh></group>)}
      {findings.filter(f=>f.type==='Road'&&f.geometry).map(f=><mesh key={f.id} position={[f.geometry!.x,.56,f.geometry!.z]} rotation={[-Math.PI/2,0,0]} raycast={noRay}><planeGeometry args={[f.geometry!.width,f.geometry!.depth]}/><meshBasicMaterial color="#e87836" transparent opacity={.92} side={THREE.DoubleSide}/></mesh>)}
    </>}
...
function World(props:SceneProps){
  ...
  const previous=useRef(memory.get('district-camera') as {position:number[];target:number[];zoom:number;mode:string;commandNonce:number}|undefined);
  ...
  useEffect(()=>{
    ...
    if(firstCommand.current){firstCommand.current=false;if(previous.current&&previous.current.mode===mode&&previous.current.commandNonce===command.nonce)return;}
    ...
    if(command.type==='block'){target.set(selected?selected.x-(compact?0:4):0,0,selected?selected.z+(compact?-3:6):0);pos.copy(target).add(new THREE.Vector3(175,320,430));zoom=compact?Math.max(3.5,Math.min(5.6,size.width/72)):8.0;}
    if(command.type==='focus'&&selected){target.set(selected.x,floor!==null?(floor+.5)*selected.floorHeight:exploded?selected.height:selected.height/2,selected.z);pos.copy(target).add(new THREE.Vector3(110,180,195));zoom=compact?Math.min(8,size.width/(selected.width*2)):(exploded?11:10);}
  ...
  useFrame((state,delta)=>{
    ...
    memory.set('district-camera',{position:camera.position.toArray(),target:ctl.target.toArray(),zoom:camera.zoom,mode,commandNonce:command.nonce});
  ...
  return <>
    ...
    <Utilities layers={layers} underground={underground} selected={selected}/>
