    ...
    const time = now(),
      i: Investigation = {
        ...
        registerSnapshot: {
          ...
          associations: d.associations,
          parcels: d.parcels,
          parcelIdentifiers: d.parcelIdentifiers,
          missing: d.missing,
        },
        findings,
        evidence: [
...
export async function exportRegister(
  ...
  const snapshot = i?.registerSnapshot
    ? {
        ...
        parcelIdentifiers: i.registerSnapshot.parcelIdentifiers ?? [],
      }
    : current;
  ...
  if (format === "csv") {
    const rows = [
      ...
      ...(d.parcelIdentifiers ?? []).map((p) => [
        ...
        p.scheme === "demo_ulpin"
          ? "Fictional - not officially issued"
          : "Source assertion",
      ]),
      ...data.register.map((r) => [
        r.kind,
  ...
  const sourceCredit = hasOsmAttribution([d.building, ...d.parcels.map((p) => p.feature)])
    ...
    : "";
  const googleCredit = hasGoogleAttribution([d.building])
    ? '<p><small>Building-outline reference: Google Research Open Buildings V3, May 2023 inference. https://sites.research.google/gr/open-buildings/ . Used under ODbL 1.0. OpenStreetMap supplies selection-boundary and road context, not these Google-predicted building outlines. No measured interiors, occupants or ownership are supplied by Google.</small></p>'
    : '';
  const attributedHtml = sourceCredit || googleCredit ? html.replace("</body>", sourceCredit + googleCredit + "</body>") : html;
  if (format === "zip")
    return sourceBundle(data, attributedHtml, d.sources, `${selection.kind}-register`);
  if (format === "pdf") return registerPdf(attributedHtml);
