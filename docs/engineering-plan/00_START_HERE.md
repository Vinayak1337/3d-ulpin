...
Read `backlog.json` for current task state and `tasks/Txxx_RESULT.md` for accepted evidence. T001's successful isolated run is recorded in its result. Inherited implementation is reusable but does not automatically satisfy whole task briefs. No later database migration, hosted run, merge, or visual acceptance is claimed without its corresponding result.
