...
Owner **FND**. Application baseline `f623cff897f91bb3ebd4c225f700ac263f7beb72`; plan revised 22 September 2026, implementation note updated 23 September 2026. Read [00](00-README.md). Implement F0 → F1-min → V0 before broad F1-feature/F2 work. The historical audit is not required reading.

Current continuation: `codex/usp-staging-continuation-20260923`, based on `origin/staging@2838e798af838d7646ff92b1120c62c8458103da`, PR #13. Old branch names below describe historical checkpoints. Assess remaining gates against [actual D0 evidence](../evidence/usp/fnd/d0/README.md).
...
> Read 00, this handoff including its F0a implementation state, root/web AGENTS and the linked actual services; check branch drift. Start an isolated continuation from the latest origin/staging, targeting staging; reuse the implemented FND ports and D0 receipts rather than rebuilding tested schemas, codecs or services. Complete the remaining F0 domain result/manifest/command/job/release contracts and ports, then F1-min using full D0 and one D1 frame sample. Own FND shared schemas/ports, exact intake/snapshot/asset refs, bounded uploads, same-client prepare/commit receipts, fenced jobs, access/release rules and additive migration registration. Keep source IDs/bytes, current wrappers and local restrictions. UI owns frontend/SSR mounts, DATA full pack files, feature owners leaves; send concrete patches, not duplicate services. Complete the live V0 seam before a broad foundation/auth platform. Run section10 producer/consumer and real-service rollback/replay/access tests and return commits, schemas, pack hashes and actual receipts. Do not ask humans to invent engineering defaults, treat contract-smoke as the full D0 neighbourhood, claim mocks prove integration, activate public services or merge main without authorization.
