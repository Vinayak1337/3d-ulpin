...
**Dataset policy update, 24 September 2026:** [H00 section 4](00-README.md#4-data-packs-acquire-before-implementing-against-imaginary-inputs) now requires geography-independent real-data scale and unfamiliar/sparse-source qualification. No Delhi/Uttam Nagar fixture is a prerequisite. Existing admission limits remain; the larger corpus targets below are planned acceptance work, not implemented throughput.
...
H00's 10,000/100,000/1,000,000-building D3 ladder describes aggregate real-source corpora across bounded jobs and spatial publications, not a new single-upload or single-scene limit. Count unique buildings separately from parts, vertices, LoDs and repeated deliveries. Before larger rungs, qualify durable corpus membership, per-stage backpressure, bounded native-format extraction and cross-part identity/recovery with FND/UI; until then report the exact supported rung and reason, not a scale pass from chunk counts alone.
...
**After core integration:** use the geography-independent D3 corpus and acquisition routes in [H00 section 4](00-README.md#4-data-packs-acquire-before-implementing-against-imaginary-inputs). Start with real 3DBAG roof geometry and suitable same-area PDOK context; use independently sourced Overture attribute layouts and Swiss native 3D delivery as candidates for sparse-schema and format qualification. Neither provider nor locality is hardcoded acceptance. [D4 DDA inventory](https://dda.gov.in/sites/default/files/Housing_Department/list_of_flats_and_garages_dda_premium_housing_scheme_2026.pdf) remains an independent document-row example, never invented polygons. Preserve raw/native inputs and any extraction recipe/lineage. Reuse the retained D1 smoke regression before extending rich geometry. Different cities remain different datasets; shared BAG/AHN ancestry is not independent evidence.
...
**Adaptive acceptance, not dataset-specific rewriting:**

* A1 — Supported-format, previously unmapped delivery: freeze application/parser code before intake. Use supplied source documentation plus bounded representative metadata/examples to propose a versioned allowlisted recipe, qualify against independently checked records, and persist it through the actual mapping workflow. The remainder must process without per-row LLM calls or application-code edits. Record initial mapping errors, corrections, unresolved facts, calls/tokens/cost and first useful preview. A source-specific hardcoded code patch or pre-renamed fixture fails this no-code-edit claim, even if it fixes ingestion.
* A2 — Holdout discipline: reserve at least two independent source-family deliveries for evaluation and distinguish initial discovery examples from unseen evaluation records. Measure correctness per mapped fact/identity/reference, false confident mappings and explicit abstentions against source-authored or independently reviewed expectations. If a family is used to repair the implementation, it becomes development data; reserve another untouched delivery for the final evaluation. Another tile from the same release is not unseen-schema evidence.
* A3 — Naturally sparse/fragmented input: absent versus null, missing height/floor/address, incomplete part/parent links, multilingual/literal keys and source-only records stay inspectable with exact evidence and per-task capabilities. Measure actual missingness; add clearly labelled authored edge cases where the real cohort does not cover a case. Known source-supported zero is distinct from missing. Optional fields must not be fabricated or all records rejected solely to avoid sparse-data handling.
* A4 — New format/profile: unsupported CityGML, GeoParquet, FileGDB or other native encodings remain retained/unsupported until a bounded deterministic parser or traceable conversion is qualified. Report adapter implementation and its tests separately from runtime recipe adaptation. No generated executable parser, automatic schema migration or relaxed validation in response to an upload. A new adapter feeds the same canonical/display lanes and shared map.
* A5 — Run the D3 aggregate ladder with missing fields, dense/sparse regions, cross-chunk/tile relationships and concurrent map inspection. Preserve the existing recovery tests below. No duplicate/lost/resurrected identity after pause/restart/replay; every source record has a reconciled disposition. Report backend corpus capacity separately from visible client geometry and concurrent-user load, which is a separate workload.
...
> Implement INGEST using 00, 01, this file and the FIND/UI producer-consumer contracts. On feat/usp-ingestion obtain chunked D0, then attempt geography-independent D3/D4 real source families and the D1 asset lane. Complete deterministic receipt→qualified mapping→fenced chunks→durable selectable draft→review-group→existing record workflow before AI or scale. Then execute H00's real 10k/100k/1M aggregate ladder within qualified per-job/display limits and J's A1–A5 unfamiliar/sparse-source cases. Do not require a locality, pre-rename holdouts into our schema, clone data to claim real scale or treat parser development as no-code-edit runtime adaptation. Build immutable manifests/tombstones and consistent status+cursor replay; use 01 guards and 99's one shared viewport. Preserve original bytes, source meanings, unknowns and current recorded data. FND/UI own shared hooks; no alternate queue/renderer or per-import training. Execute J real-service failure/stream/geometry tests and return pack hashes, actual assets/receipts/timings and explicit unqualified capabilities. Use the stated fallbacks, not invented values or repeated human clarification. No deployment or main merge without authorization.
