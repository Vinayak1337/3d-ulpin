...
**Add records** supports a new evidenced draft, an explicit import of a built workspace with the same frame/benchmark, and a new synthetic site. **Prepare source inputs** opens the existing workbench in the site's frame. New rights must be entered with a source revision and locator; arbitrary imported geometry never creates a party or right. The Nandan seed alone uses its authored fictional rights schedule.
